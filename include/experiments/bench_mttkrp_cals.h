...
BenchResult benchmark_mttkrp_cals(int rank, int mode, vector<dim_t> modes, F &&f, cals::mttkrp::MttkrpParams &params) {
  ...
  for (auto i = 0; i < ITERATIONS; i++) {
    ...
    time.push_back(params.mttkrp_timers.timers[MttkrpTimers::TIMERS::MT_KRP].get_time() +
  ...
  return {*std::min_element(time.begin(), time.end()), *std::min_element(f_time.begin(), f_time.end()),
          *std::min_element(s_time.begin(), s_time.end()), params.flops, params.memops};
}
} // namespace cals::bench
