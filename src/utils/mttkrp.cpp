...
void khatri_rao_cuda(double *A, double *B, unsigned int IA, unsigned int IB, unsigned int JK, double *K);
...
MttkrpLut read_lookup_table(vector<dim_t> const &modes, int threads, bool gpu, bool suppress_warning) {
  ...
  std::string dir{};
  if (gpu) {
    dir = string(SOURCE_DIR) + "/data/GPU" + "/lookup_tables/" + cals::utils::mode_string(modes) + "/";
  } else {
    std::string blas_name = CALS_BACKEND;
    dir = string(SOURCE_DIR) + "/data/" + blas_name + "/lookup_tables/" + cals::utils::mode_string(modes) + "/" +
  ...
  for (auto m = 0lu; m < modes.size(); m++) {
    ...
    if (file.is_open()) {
      ...
      if (!suppress_warning)
        std::cout << "Lookup table missing for tensor: " << utils::mode_string(modes) << ", Threads: " << threads
                  << std::endl;
      return MttkrpLut();
    }
  }
...
#pragma omp parallel for // NOLINT(openmp-use-default-none)
    ...
    params.memops +=
        1llu * K.get_cols() * K.get_rows() + 1llu * A.get_rows() * A.get_cols() + 1llu * B.get_rows() * B.get_cols();
    return K;
  }
#else
  //#if CUDA_ENABLED
...
khatri_rao_rec(std::stack<Matrix *> &remaining_targets, vector<Matrix> &workspace, int w_index, KrpParams &params) {
  if (!remaining_targets.empty()) {
    ...
    if (params.cuda) {
      ...
      khatri_rao_cuda(A.get_cudata(), B->get_cudata(), A.get_rows(), B->get_rows(), K.get_cols(), K.get_cudata());
...
Matrix &khatri_rao(Ktensor &u, vector<Matrix> &workspace, dim_t mode, KrpParams &params) {
  ...
  if (params.cuda) {
    ...
    auto &stream = params.cuda_params.streams[0];
    cublasSetStream(handle, stream);
    khatri_rao_cuda((*u0).get_cudata(), (*u1).get_cudata(), (*u0).get_rows(), (*u1).get_rows(),
                    workspace[w_index].get_cols(), workspace[w_index].get_cudata());
    cudaStreamSynchronize(stream); // Make sure KRP is done
#else
    std::cerr << "Not compiled with CUDA support" << std::endl;
    exit(EXIT_FAILURE);
...
Matrix &mttkrp_impl(const Tensor &X, Ktensor &u, vector<Matrix> &workspace, int mode, MttkrpParams &params) {
  ...
  if (params.cuda) {
    ...
    for (auto block_idx = 0; block_idx < unfolding.n_blocks; block_idx++) {
      ...
      if (mode == 0)
        cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, G.get_rows(), G.get_cols(), unfolding.cols, &one, X_mode_blk_ptr,
                    ldX_mode, krp_blk_ptr, krp.get_col_stride(), &zero, G.get_cudata(), G.get_col_stride());
      else
        cublasDgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, G.get_rows(), G.get_cols(), unfolding.cols, &one, X_mode_blk_ptr,
                    ldX_mode, krp_blk_ptr, krp.get_col_stride(), &one, G.get_cudata(), G.get_col_stride());
    }
    ...
    for (dim_t block_idx = 0; block_idx < unfolding.n_blocks; block_idx++) {
      ...
      if (mode == 0)
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, G.get_rows(), G.get_cols(), unfolding.cols, 1.0,
                    X_mode_blk_ptr, ldX_mode, krp_blk_ptr, krp.get_col_stride(), 0.0, G.get_data(), G.get_col_stride());
      else
        cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, G.get_rows(), G.get_cols(), unfolding.cols, 1.0,
                    X_mode_blk_ptr, ldX_mode, krp_blk_ptr, krp.get_col_stride(), 1.0, G.get_data(), G.get_col_stride());
    }
  }
  ...
  params.flops = 1llu * unfolding.n_blocks *
                 (2llu * G.get_rows() * G.get_cols() * unfolding.cols + 2llu * G.get_rows() * G.get_cols());
  params.memops = 1llu * unfolding.n_blocks *
                  (1llu * G.get_rows() * unfolding.cols + 1llu * G.get_cols() * unfolding.cols +
...
Matrix &mttkrp_twostep_impl(const Tensor &X,
  ...
  if (params.cuda) {
    ...
    for (auto blk = 0; blk < gemm_params.n_blocks; blk++) {
      ...
      cublasDgemm(handle, opA, CUBLAS_OP_N, gemm_params.block_rows, workspace.get_cols(), B.get_rows(), &one,
                  X.get_cudata() + blk * gemm_params.block_offset,
                  gemm_params.stride, // careful with the stride. Is it always this?
                  B.get_cudata(), B.get_col_stride(), &zero, workspace.get_cudata() + blk * gemm_params.block_rows,
                  workspace.get_col_stride());
    }
...
#pragma omp parallel for // NOLINT(openmp-use-default-none)
    for (dim_t blk = 0; blk < gemm_params.n_blocks; blk++)
      cblas_dgemm(CblasColMajor, gemm_params.trans_A, CblasNoTrans, gemm_params.block_rows, workspace.get_cols(),
                  B.get_rows(), 1.0, X.get_data() + blk * gemm_params.block_offset,
                  gemm_params.stride, // careful with the stride. Is it always this?
                  B.get_data(), B.get_col_stride(), 0.0, workspace.get_data() + blk * gemm_params.block_rows,
                  workspace.get_col_stride());
  }
  ...
  if (params.cuda) {
    ...
    for (auto col = 0; col < workspace.get_cols(); col++) {
      ...
      if (flag) {
        cublasDgeam(handle, CUBLAS_OP_T, CUBLAS_OP_N, gemv_params.A_rows, gemv_params.A_cols, &one,
                    workspace.get_cudata() + col * workspace.get_col_stride(), gemv_params.A_cols, &zero,
                    temp + col * workspace.get_col_stride(), gemv_params.A_rows,
                    temp + col * workspace.get_col_stride(), gemv_params.A_rows);
        cublasDgemv(handle, opA, gemv_params.A_rows, gemv_params.A_cols, &one, temp + col * workspace.get_col_stride(),
                    gemv_params.A_rows, x.get_cudata() + col * x.get_col_stride(), 1, &zero,
        ...
        cublasDgemv(handle, opA, gemv_params.A_rows, gemv_params.A_cols, &one,
                    ...
                    x.get_cudata() + col * x.get_col_stride(), 1, &zero, y.get_cudata() + col * y.get_col_stride(), 1);
    }
...
#pragma omp parallel for // NOLINT(openmp-use-default-none)
    for (dim_t col = 0; col < workspace.get_cols(); col++)
      cblas_dgemv(CblasColMajor, gemv_params.trans_A, gemv_params.A_rows, gemv_params.A_cols, 1.0,
                  ...
                  x.get_data() + col * x.get_col_stride(), 1, 0.0, y.get_data() + col * y.get_col_stride(), 1);
  }
  ...
  params.memops = 1llu * gemm_params.n_blocks *
                  (1llu * gemm_params.block_rows * B.get_rows() + 1llu * B.get_rows() * workspace.get_cols() +
...
Matrix &mttkrp(const Tensor &X, Ktensor &u, vector<Matrix> &workspace, dim_t mode, MttkrpParams &params) {
  ...
  if (X.get_n_modes() != 3)
    ...
    if (params.method == MTTKRP_METHOD::MTTKRP)
      mttkrp_impl(X, u, workspace, mode, params);
    else if ((params.method == MTTKRP_METHOD::TWOSTEP0) || (params.method == MTTKRP_METHOD::TWOSTEP1))
      ...
      } else // If LUT does not exist, revert to common sense defaults
      {
        if (params.cuda) // If CUDA, MTTKRP is the best alternative
          mttkrp_impl(X, u, workspace, mode, params);
        else if (get_threads() != 1) // If multi-threaded, never Twostep0
        {
          if (mode == 1)
            mttkrp_impl(X, u, workspace, mode, params);
          else {
